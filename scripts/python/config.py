...
PATHS = {
    ...
    'BUTTONS': os.path.join(ROOT_DIR, 'graphics', 'buttons'),  # Assumed subdir for button images like sloth.png; adjust if directly in graphics/
    'SOUNDCLIPS': os.path.join(ROOT_DIR, 'soundclips'),
    'BASE': os.path.join(ROOT_DIR, 'photogrammetry'),  # Output base dir (was d:\photogrammetry)
    'INIT_PHOTOS': os.path.join(ROOT_DIR, 'graphics', 'initialization'),
    # Add more as needed, e.g., 'REF_IMAGE': os.path.join(ROOT_DIR, 'graphics', 'preview_photo.jpg')
}
